...
namespace Valeo.Domain.Common
{
    public class FileUpload
    {
        ...
        /// <returns></returns>
        //public int UploadFile(HttpPostedFileBase fileName, string imgName)
        public List<string> UploadFile(List<HttpPostedFileBase> ListName, int rootFlag, string modelDir, bool isClear = false)
        {
            int index = -1;
            string sourceFileName = "";
            string destFileName = "";
            string Geng = @"" + ConfigurationManager.AppSettings["Valeo.SystemFile"]; ;
            string rootFilePath = @"";
            ...
            try
            {
                if (rootFlag == 0)
                {
                    rootFilePath = @"SystemFile";
                }
                else
                {
                    rootFilePath = @"DataFile";
                }
                if (ListName.Count > 0)
                {
                    //文件更目录
                    rootFilePath += String.Format(@"\{0}\{1}\{2}\{3}\"
                                            , dtNow.Year, dtNow.ToString("yyyyMM")
                                            , modelDir, dtNow.ToString("yyyyMMdd"));
                }
                if (!Directory.Exists(Geng + rootFilePath))
                {
                    Directory.CreateDirectory(Geng + rootFilePath);
                }
                else if (isClear)
                {
                    //清空文件目录数据
                    DeleteFiles(Geng + rootFilePath);
                }

                index = 0;
                foreach (HttpPostedFileBase file in ListName)
                {
                    index += 1;

                    if (string.Compare(file.ContentType, "image/bmp", true) == 0
                        || string.Compare(file.ContentType, "image/jpg", true) == 0
                        || string.Compare(file.ContentType, "image/jpeg", true) == 0
                        || string.Compare(file.ContentType, "image/png", true) == 0
                        || string.Compare(file.ContentType, "image/tiff", true) == 0)
                    {
                        string FileName = string.Format(@"{0}{1}{2}", dtNow.ToString("HHmmss"), index, System.IO.Path.GetExtension(file.FileName));
                        destFileName = Geng + rootFilePath + FileName;
                        file.SaveAs(destFileName);
                        lstFileName.Add(rootFilePath + FileName);
                    }
                    else
                    {
                        lstFileName.Add("");
            ...
            //try
            //{
            //    string FilePath = System.Web.HttpContext.Current.Server.MapPath("/App_Data/File/");//保存路径
            ...
            //        return ErrMessage;
            //    }
            //    //string FilePath = System.Web.HttpContext.Current.Server.MapPath("/App_Data/File/");
